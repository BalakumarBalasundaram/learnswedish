
# Swedish Nominal Phrase – Master Pattern Exercise

## Introduction: The Master Pattern

> 🧠 **Mental model — the 8-floor noun elevator:** every noun rides the same elevator, always in the same order, ground floor to top.
>
> ```
>  8 │ de + adj + plural-definite   (de stora böckerna)
>  7 │ plural definite              (böckerna)
>  6 │ adj + plural indefinite      (stora böcker)
>  5 │ plural indefinite            (böcker)
>  4 │ den/det + adj + definite     (den stora boken)
>  3 │ definite                     (boken)
>  2 │ en/ett + adjective           (en stor bok)
>  1 │ en/ett indefinite            (en bok)
>    └───────────────────────────▶ same noun, 8 forms, one fixed order
> ```

## 📜 Background — why definiteness is marked *twice* ("double definiteness")

Rows 4 and 8 of the elevator (**den stora boken**, **de stora böckerna**) look redundant at first — why mark "the" both with a separate word (`den`/`de`) *and* the `-en`/`-na` suffix? This is a real historical layering, not a mistake: the suffix (see [04_bestämningsord.md](04_bestämningsord.md)) fossilized first from an Old Norse demonstrative. Later, once an adjective was added in front of the noun, spoken Swedish needed a *second* definiteness marker positioned before the adjective, so speakers recruited another demonstrative (`den/det/de`) to do the job — otherwise the adjective would sit outside the "definite bubble" and sound unmarked. Norwegian and (spoken) Danish show the same **double definiteness** pattern; it's a shared North Germanic innovation that written Danish, German, and English never fully developed.

This exercise trains the **core Swedish nominal phrase system**.
For every noun, practice the following transformation path:

1. **Indefinite singular**  
2. **Indefinite singular + adjective**  
3. **Definite singular**  
4. **Definite singular + adjective**  
5. **Indefinite plural**  
6. **Indefinite plural + adjective**  
7. **Definite plural**  
8. **Definite plural + adjective**

### Universal rules to remember
- **en/ett** depends on the noun
- **Plural → adjective ends in -a**
- **Definite + adjective → den / det / de**
- **Plural definite → -na / -en on the noun**

### Example patterns

**EN-word (bok):**  
en bok → boken → den stora boken → böcker → böckerna → de stora böckerna

**ETT-word (barn):**  
ett barn → barnet → det lilla barnet → barn → barnen → de små barnen

---

## Exercise List – 300 Nouns

Use EACH noun below with the master pattern.

### 1–50 (Everyday & home)
**Pattern:** indefinite singular → indefinite + adjective → definite singular → den/det + adjective + definite singular → indefinite plural → adjective + indefinite plural → definite plural → de + adjective + definite plural

1. bok → en bok → en stor bok → boken → den stora boken → böcker → stora böcker → böckerna → de stora böckerna
2. stol -> en stol -> en bekväm stol -> stolen -> den bekväma stolen -> stolar -> bekväma stolar -> stolarna -> de bekväma stolarna
3. bord -> ett bord -> ett runt bord -> bordet -> det runda bordet -> bord -> runda bord -> borden -> de runda borden
4. penna -> en penna -> en blå penna -> pennan -> den blå pennan -> pennor -> blå pennor -> pennorna -> de blå pennorna
5. kopp -> en kopp -> en fin kopp -> koppen -> den fina koppen -> koppar -> fina koppar -> kopparna -> de fina kopparna
6. tallrik -> en tallrik -> en fin tallrik -> tallriken -> den fina tallriken -> tallrikar -> fina tallrikar -> tallrikarna -> de fina tallrikarna
7. sked 
8. gaffel -> en gaffel -> en vass gaffel -> gaffeln -> den vassa gaffeln -> gafflar -> vassa gafflar -> gafflarna -> de vassa gafflarna
9. kniv -> en kniv -> en vass kniv -> kniven -> den vassa kniven -> knivar -> vassa knivar -> knivarna -> de vassa knivarna
10. telefon -> en telefon -> en mobiltelefon -> telefonen -> den mobiltelefonen -> telefoner -> mobiltelefoner -> telefonerna -> de mobiltelefonerna
11. lampa
12. dörr
13. fönster -> ett fönster -> ett stort fönster -> fönstret -> det stora fönstret -> fönster -> stora fönster -> fönstren -> de stora fönstren
14. säng
15. kudde
16. filt
17. spegel
18. väska
19. nyckel
20. klocka
21. skåp
22. hylla
23. låda
24. matta
25. gardin
26. tv
27. radio
28. dator
29. skärm
30. mus
31. laddare
32. kabel
33. tidning
34. brev
35. kuvert
36. flaska
37. glas
38. burk
39. påse
40. bröd
41. smör
42. ost
43. frukt
44. grönsak
45. kaka
46. socker
47. salt
48. vatten
49. mjölk
50. kaffe

### 51–100 (Preschool & school)
51. barn
52. elev
53. pedagog
54. lärare
55. klass
56. grupp
57. rum
58. sal
59. skola
60. förskola
61. leksak
62. boll
63. bil
64. docka
65. kloss
66. pussel
67. spel
68. bokhylla
69. matta
70. kudde
71. stol
72. bord
73. whiteboard
74. penna
75. sudd
76. papper
77. bokstav
78. siffra
79. uppgift
80. övning
81. schema
82. rast
83. lektion
84. matsal
85. frukost
86. lunch
87. mellanmål
88. aktivitet
89. vila
90. samling
91. sång
92. saga
93. berättelse
94. fråga
95. svar
96. regel
97. tur
98. kö
99. plats
100. hörna

### 101–150 (Body & health)
101. huvud
102. arm
103. hand
104. finger
105. ben
106. fot
107. tå
108. knä
109. axel
110. rygg
111. mage
112. hals
113. mun
114. näsa
115. öga
116. öra
117. tand
118. tunga
119. hud
120. hår
121. hjärta
122. blod
123. hosta
124. feber
125. smärta
126. vila
127. sömn
128. mat
129. dryck
130. medicin
131. tablett
132. plåster
133. sår
134. läkare
135. sjuksköterska
136. sjukhus
137. klinik
138. hälsa
139. träning
140. rörelse
141. promenad
142. andning
143. stress
144. lugn
145. oro
146. känsla
147. glädje
148. rädsla
149. trötthet
150. energi

### 151–200 (City & society)
151. gata
152. väg
153. trottoar
154. park
155. torg
156. bro
157. tunnel
158. station
159. hållplats
160. spår
161. tåg
162. spårvagn
163. cykel
164. skylt
165. trafik
166. ljus
167. lampa
168. polis
169. brand
170. ambulans
171. sjukhus
172. apotek
173. butik
174. affär
175. marknad
176. bank
177. post
178. paket
179. brev
180. kontor
181. kommun
182. myndighet
183. tjänst
184. hjälp
185. stöd
186. beslut
187. regel
188. lag
189. rätt
190. ansvar
191. plan
192. projekt
193. möte
194. dokument
195. ansökan
196. blankett
197. information
198. service
199. system
200. samhälle

### 201–300 (Work, nature & abstract)
201. arbete
202. jobb
203. yrke
204. chef
205. kollega
206. arbetsplats
207. möte
208. schema
209. uppgift
210. ansvar
211. lön
212. paus
213. rast
214. kontrakt
215. projekt
216. plan
217. rapport
218. dator
219. program
220. system
221. fil
222. mapp
223. lösenord
224. konto
225. nätverk
226. server
227. kabel
228. signal
229. data
230. information
231. teknik
232. maskin
233. verktyg
234. skruv
235. spik
236. hammare
237. borr
238. mätare
239. sensor
240. kontroll
241. test
242. resultat
243. problem
244. lösning
245. förbättring
246. förändring
247. risk
248. säkerhet
249. kvalitet
250. ansvar
251. skog
252. träd
253. gren
254. blad
255. blomma
256. gräs
257. jord
258. sand
259. sten
260. vatten
261. sjö
262. hav
263. flod
264. regn
265. snö
266. vind
267. sol
268. himmel
269. moln
270. ljus
271. mörker
272. tid
273. dag
274. natt
275. morgon
276. kväll
277. framtid
278. nutid
279. dåtid
280. början
281. slut
282. förändring
283. möjlighet
284. risk
285. idé
286. tanke
287. mening
288. språk
289. ord
290. grammatik
291. struktur
292. mönster
293. exempel
294. regel
295. undantag
296. system
297. process
298. utveckling
299. lärande
300. förståelse
